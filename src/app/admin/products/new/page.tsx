...
import { Camera, Video, Loader2, ArrowLeft } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex. Aura Silk Dress"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-neutral-400 mb-1.5" htmlFor="price">Price ($) *</label>
                <input
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-400 mb-1.5" htmlFor="quantity">Quantity Available *</label>
                <input
                  id="quantity" name="quantity" type="number" min="0" required
                ...
                <div className="flex flex-wrap gap-3">
                  {images.map((file, i) => (
                    <div key={i} className="relative w-24 h-24 rounded-lg overflow-hidden border border-neutral-700">
                      ...
                      <button type="button" onClick={() => removeFile(i, 'image')} className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1 hover:bg-black">
                        &times;
                      </button>
                    </div>
                  ))}
                ...
                <div className="flex flex-wrap gap-3">
                  {videos.map((file, i) => (
                    <div key={i} className="relative w-24 h-24 rounded-lg overflow-hidden border border-neutral-700 bg-black flex items-center justify-center">
                      ...
                      <button type="button" onClick={() => removeFile(i, 'video')} className="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1 hover:bg-black">
                        &times;
                      </button>
                    </div>
                  ))}
          ...
          <div className="pt-4 border-t border-neutral-800">
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center py-3 bg-white text-black font-semibold rounded-lg hover:bg-neutral-200 focus:outline-none focus:ring-2 focus:ring-white/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
