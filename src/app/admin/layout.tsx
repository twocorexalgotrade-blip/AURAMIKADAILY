...
export default async function AdminLayout({
  ...
  // Simple auth check - requires ADMIN_PASSWORD to match the cookie token
  if (!token || token.value !== process.env.ADMIN_PASSWORD) {
    // We redirect to login if not authenticated
    // Wait, the login page will also be under /admin, so we need to exclude it or put login outside
    // For simplicity, we can do it in middleware, but if we do it here:
    ...
    <div className="flex h-screen bg-neutral-950 text-neutral-100">
      ...
      <aside className="w-64 border-r border-neutral-800 bg-neutral-950 flex flex-col">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          ...
          <Link href="/admin/products" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-neutral-400 hover:text-white hover:bg-neutral-900 transition-colors bg-neutral-900/50">
            <Package className="w-5 h-5" />
            Products
          </Link>
          <Link href="/admin/settings" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-neutral-400 hover:text-white hover:bg-neutral-900 transition-colors">
